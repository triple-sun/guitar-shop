import { Entity, fillObject, getRemovedMessage, OrderEntity } from '@guitar-shop/core';
...
export class OrderService {
  ...
  async findAll(dto: OrderQueryDto) {
    ...
    if (maxPrice || minPrice) {
      return rdo.filter(
        (order) =>
          order.orderPrice > minPrice &&
          (maxPrice ? order.orderPrice < maxPrice : true)
      );
    }
  ...
  async remove(id: number) {
    ...
    return getRemovedMessage(Entity.Order, id);
  }
}
